...
class PackedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        print(f'process rank : {self._process_rank}')
        ...
        print(f'File names inside packedDataset initial: {self._filenames}')
        print(f'Shard Id :{shard_id} , max num files :{max_num_files}, num shards : {num_shards}')
        filenames = self._filenames[shard_id:max_num_files:num_shards]
        print(f'File names inside packedDataset: {filenames}')
...
class PackedDatasetIterator:
    def __init__(self, filenames, n_chunks, block_size, seed, shuffle, wrap):
        ...
        self._block_idxs = []
        self._curr_idx = 0
        print(f'PACKED DATASET ITERATOR: {self._filenames}')
    ...
    def _load_n_chunks(self):
        ...
        if self._n_chunks > len(self._filenames[self._file_idx :]):
            print(self._n_chunks)
            print(self._filenames)
            print(self._file_idx)
            print(len(self._filenames[self._file_idx :]))
            print(self._wrap)
                  
            if not self._wrap:
                print('Yaheen hai error')
                raise StopIteration
            self._file_idx = 0
...
class CombinedDatasetIterator:
    def __init__(self, datasets, seed, weights):
        #print(datasets)
        #print(f"test: {iter(datasets[0])}")
        #print(f"test: {datasets[0][0]}")
        #print(f"test: {next(datasets[0])}")
...
class LLMGeneratedDataset(torch.utils.data.IterableDataset):
    def __init__(self, tokenizer, model, vocab_words, block_size, num_samples=10000, seed=42):
        ...
        self.device = model.device  # Store the device of the model
    ...
    def __iter__(self):
        for _ in range(self.num_samples):
            ...
            inputs = {k: v.to(self.device) for k, v in inputs.items()}  # Move input tensors to the model's device
            with torch.no_grad():

                outputs = self.model.generate(**inputs, max_length=2048, pad_token_id=self.tokenizer.pad_token_id)
...
#             inputs = self.tokenizer(word, return_tensors="pt", return_attention_mask=False)
#             with torch.no_grad():
#                 outputs = self.model.generate(**inputs, max_length=self.block_size, pad_token_id=self.tokenizer.pad_token_id).to('cuda:1')
#             text = self.tokenizer.decode(outputs[0], skip_special_tokens=True)
#             tokenized_text = self.tokenizer(text, max_length=self.block_size, padding="max_length", truncation=True, return_tensors="pt")
